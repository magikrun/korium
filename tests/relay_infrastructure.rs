...
async fn publish_address_idempotent() {
    ...
    // Publish addresses
    let addrs = vec!["10.0.0.1:5000".to_string(), "192.168.1.1:5000".to_string()];
...
async fn relay_endpoint_address_format() {
    ...
    if let Some(relay_ep) = node.relay_endpoint().await {
        ...
        let primary = relay_ep
            .addrs
            .first()
            .expect("should have at least one addr");
...
async fn relay_server_shares_socket() {
    ...
    assert_eq!(
        relay_addr.port(),
        quic_addr.port(),
        "relay and QUIC should share port"
    );
}
...
async fn connect_with_relay_available() {
    ...
    node1
        .bootstrap(&relay_id, &relay_addr)
        .await
        .expect("node1 bootstrap failed");
    ...
    node1.publish_address(addrs).await.expect("publish failed");
    ...
    let result = timeout(TEST_TIMEOUT, node2.connect(&node1.identity())).await;
...
async fn smartsock_peer_registration() {
    ...
    let _ = timeout(TEST_TIMEOUT, node2.connect(&node1_id)).await;
...
async fn relay_endpoint_is_ephemeral_unsigned() {
    ...
    assert!(
        ...
        "relay_endpoint should be unsigned (ephemeral)"
    );
    assert_eq!(
        contact.timestamp, 0,
        "relay_endpoint should have zero timestamp"
    );
    ...
    assert!(
        peer_ep.signature.is_empty(),
        "peer_endpoint is also unsigned locally"
    );
}
...
async fn signed_contact_usable_for_connection() {
    ...
    node1
        .bootstrap(&hex::encode(relay_id.as_bytes()), &relay_addr)
        .await
        .expect("node1 bootstrap failed");
    node2
        .bootstrap(&hex::encode(relay_id.as_bytes()), &relay_addr)
    ...
    relay
        .publish_address(vec![relay_addr.clone()])
        .await
        .expect("relay publish failed");
...
async fn discover_relays_empty_without_mesh() {
    ...
    let relays = node
        .discover_relays()
        .await
        .expect("discover_relays should not fail");

    // Should return empty list (no mesh peers)
    assert!(
        relays.is_empty(),
        "should have no relays without mesh peers"
    );
}
...
async fn discover_relays_finds_mesh_peers() {
    ...
    relay
        ...
        .await
        .expect("relay subscribe failed");
    node1
        .subscribe("test-topic")
        .await
        .expect("node1 subscribe failed");
    node2
        .subscribe("test-topic")
        .await
        .expect("node2 subscribe failed");
    ...
    relay
        ...
        .expect("relay publish failed");
    node1
        .publish_address(vec![node1.local_addr().unwrap().to_string()])
        .await
        .expect("node1 publish failed");
    progress(start, "addresses published");
    ...
    node1
        .publish("test-topic", b"hello".to_vec())
        .await
        .expect("publish failed");
    tokio::time::sleep(Duration::from_millis(100)).await;
    ...
    assert!(
        relays.len() <= 2,
        "should find at most 2 other peers as relays"
    );
}
...
async fn discover_relays_dht_fallback() {
    ...
    node.bootstrap(&relay_id, &relay_addr)
        .await
        .expect("bootstrap failed");
    relay
        .publish_address(vec![relay_addr.clone()])
    ...
    progress(
        start,
        &format!("discovered {} relays via DHT fallback", relays.len()),
    );
...
async fn discover_relays_requires_direct_addrs() {
    ...
    for relay in &relays {
        assert!(
            relay.has_direct_addrs(),
            "relay should have direct addresses"
        );
    }
}
...
async fn relay_tunnel_machinery_verification() {
    ...
    peer.bootstrap(&relay_id, &relay_addr)
        .await
        .expect("bootstrap failed");
    progress(start, "bootstrap complete");
    ...
    let tunnel_result = smartsock
        .add_relay_tunnel(&fake_peer_id, session_id, relay_socket)
        .await;
    ...
    assert!(
        is_relayed_after,
        "peer should be relayed after activating relay path"
    );
    ...
    let switched_to_direct = smartsock
        .use_direct_path(&fake_peer_id, fake_direct_addr)
        .await;
    ...
    assert!(
        !is_relayed_final,
        "peer should not be relayed after switching to direct"
    );
...
async fn relay_tunnel_limit_enforced() {
    ...
    for i in 0..8u8 {
        let session_id = [i; 16];
        let result = smartsock
            .add_relay_tunnel(&peer_id, session_id, relay_addr)
            .await;
    ...
    let result = smartsock
        .add_relay_tunnel(&peer_id, session_id_over_limit, relay_addr)
        .await;
